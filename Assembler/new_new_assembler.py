...
def replace_if_labels(operand, labels, offset):
    ...
    if '(' in operand or ')' in operand:
        brackets = True
        operand_content = operand[1:-1]
    ret = operand_content # Si no hay label retorna tal cual
    for label in labels.keys():
        if operand_content == label:
            ret = str(int(labels[label]) + offset)
    if brackets: #Devuelve los brackets
        ret = f'({ret})'
    return ret 
...
def process_instructions(instructions, variables, labels, offset):
    ...
    for instruction in instructions:
        ...
        if len(ret) == 2:  # Instrucción con 1 operando
            ...
            # Aquí se reemplazan por su valor sin offset
            # y se le suma el offset
            if 'J' in name or 'CALL' == name:
                op = replace_if_labels(op, labels, offset = offset)
            else: #A inc no le suma offset
                op = replace_if_labels(op, labels, offset = 0)
        ...
        if len(ret) == 3:  # Instrucción con 2 operandos
            ...
            op1 = replace_if_labels(op1, labels, offset = 0)
            op2 = replace_if_labels(op2, labels, offset = 0)
...
def get_opcodes(code, CODE_indicator, DATA_indicator):
    ...
    labels, instructions = get_labels_instructions(instructions_and_labels)
    total_instructions = initial_instructions + instructions
    ...
    processed_instructions = process_instructions(
        total_instructions, variables, labels, offset)
